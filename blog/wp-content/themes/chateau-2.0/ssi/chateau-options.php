			...
			<tbody>
				...
				<tr valign="top">
					...
					<?php foreach($tcstyleArray as $key => $value): ?>
						<p>
							<input type="radio" name="t-c-style" value="<?=$key?>" id="t-c-style-<?=$key?>"<?php if($tcstyle==$key) echo ' checked'; ?> />
							<label for="t-c-style-<?=$key?>"><?=$value?></label>
						</p>
					<?php endforeach; ?>
					</td>
				</tr>
				<tr valign="top">
					...
					<td style="padding:25px 0 20px; border-bottom:1px solid #CCC;">
						...
						<?php foreach($tsideposArray as $key => $value): ?>
							<p>
								<input type="radio" name="t-side-position" value="<?=$key?>" id="t-side-position-<?=$key?>"<?php if($tcpos==$key) echo ' checked'; ?> />
								<label for="t-side-position-<?=$key?>"><?=$value?></label>
							</p>
						<?php endforeach; ?>
					</td>
				</tr>
				<tr valign="top">
					...
					<td style="padding:25px 0 20px; border-bottom:1px solid #CCC;">
						<p>
							<input type="text" name="t-accent-color" id="t-accent-color" value="<?=get_option( 't-accent-color' );?>" style="padding:5px; color:#333;" />
						</p>
					</td>
				</tr>
				<tr valign="top">
					...
					<td style="padding:25px 0 20px; border-bottom:1px solid #CCC;">
						<p>
							<input type="text" name="t-articles" id="t-articles" style="padding:5px; color:#333;" value="<?=get_option( 't-articles' );?>" />
						</p>
					</td>
				</tr>
				<tr valign="top">
					...
					<td style="padding:25px 0 20px; border-bottom:1px solid #FFF;">
						...
						<?php foreach($tcolumnsArray as $key => $value): ?>
							<p>
								<input type="radio" name="t-columns" value="<?=$key?>" id="t-columns-<?=$key?>"<?php if($tcolumns==$key) echo ' checked'; ?> />
								<label for="t-columns-<?=$key?>"><?=$value?></label>
							</p>
						<?php endforeach; ?>
					</td>
				</tr>
				...
				<tr valign="top">
					...
					<td style="padding:25px 0 20px; border-bottom:3px solid #DDD;">
						...
						<?php foreach($tsocitemsArray as $key => $value): ?>
							<p>
								<input type="checkbox" name="t-social-items[]" value="<?=$key?>" id="t-social-items-<?=$key?>"<?php if(is_array($tsocitems)): 
								if( in_array( $key, $tsocitems ) ): echo ' checked'; endif; endif; ?> />
								<label for="t-social-items-<?=$key?>"><?=$value?></label>
							</p>
						<?php endforeach; ?>
					</td>
				</tr>
